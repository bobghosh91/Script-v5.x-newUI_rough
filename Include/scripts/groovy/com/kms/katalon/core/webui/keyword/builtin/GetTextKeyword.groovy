	...
	public String getText(TestObject to, FailureHandling flowControl) throws StepFailedException {
		return WebUIKeywordMain.runKeyword({
			...
			try {
				...
				//custom code start
				String textCaseIdentifier = element.getCssValue("text-transform")

				println ("------Actual Text: " + text)

				if (textCaseIdentifier.equals("uppercase")) {
					text = text.toUpperCase()
				}
				else if (textCaseIdentifier.equals("lowercase")) {
					text = text.toLowerCase()
				}
				else if (textCaseIdentifier.equals("capitalize")) {
